...
bool read_src_data(const std::string filename, Mesh& mesh, const std::string label_filename, std::vector<int>& src_label)
{

    std::ifstream fin(label_filename.c_str());
    if (!fin.good())
    {
       std::cout<<label_filename + " is not exists"<<std::endl;
       exit(0);
    }
    std::string s;
    while(std::getline(fin, s))
    {
        src_label.push_back(std::atoi(s.c_str()));
    }
    fin.close();
    
    OpenMesh::IO::Options opt_read = OpenMesh::IO::Options::VertexColor;
    mesh.request_vertex_normals();
    bool read_OK = OpenMesh::IO::read_mesh(mesh, filename,opt_read);
