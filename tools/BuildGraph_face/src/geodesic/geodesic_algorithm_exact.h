...
namespace geodesic {
	...
	public:
        ...
        GeodesicAlgorithmExact(Mesh* mesh, size_t src, Scalar m_radius, Scalar radio)
        {
            ori_mesh = mesh;
            Mesh* sub_mesh = new Mesh;
            bool ok = construct_submesh(sub_mesh, src, m_radius);
            ...
            {
                // std::cout << "Warning: Some points cannot be covered under the specified radius, auto increase the radius" << std::endl;
                int increase_num = 0;
                int max_num = 10;
                while(!ok)
                {
                    m_radius = radio * m_radius;
                    ok = construct_submesh(sub_mesh, src, m_radius);
                    increase_num ++;
                    if(increase_num>max_num)
                    {
                        // std::cerr << "Error: automatic increase is invalid, Check for isolated points and delete them!" << std::endl;
                        exit(1);
                    }
                }
                // std::cout << " increase radius to " << m_radius << " increase_num is " << increase_num << std::endl;
                GeodesicAlgorithmBase::initialize(sub_mesh);
                m_memory_allocator.reset(sub_mesh->n_edges(), sub_mesh->n_edges());
//                exit(1);
            }
